...
class SafeImageLoader extends Component {
    constructor(props) {
        ...
        this.overrideStyle = props.overrideStyle || false;
            ...
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    xhr.onreadystatechange = function() {
                        if (xhr.readyState === XMLHttpRequest.DONE) {
                            if (xhr.status === 200) {
                                let localUrl = URL.createObjectURL(
                                    xhr.response
                                );
                                self.setState({
                                    loading: false,
                                ...
                                if (newUrl == DEFAULT_404_ROUTE) {
                                    ...
                                    if (self.transparent) {
                                        newUrl = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8Xw8AAoMBgDTD2qgAAAAASUVORK5CYII=";
                                    }
                                }
    ...
    render() {
        if (this.state.error) return <img src="" alt="illust" />;
        ...
        if (this.downloadable)
            return (
                <div align="center">
                    <Button
                        onClick={this.openAssetInNewPage.bind(this)}
                        variant="contained"
                        endIcon={<DownloadIcon></DownloadIcon>}
                    >
                        {strings.getString("STILL_ENTRY_DOWNLOAD_ARTWORK")}
                    </Button>
                    <Divider></Divider>
                    <br></br>
                    <img
