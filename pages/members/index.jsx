...
class DjMembersListPage extends AbstractList {
    ...
    getIllustUrl(member) {
        return consts.cdn + "ondemand/character/character_stand_up_0" + member.Id + ".png"
    }
    ...
    renderElements() {
        ...
        for (let id in units) {
            ...
            const unitMembers = id === "50" ? Object.entries(members).filter(([_,m]) => m.Unit == id).map(e => e[0]): unit.InitDeckCharacterIds.sort();
                    ...
                    <Grid item xs={12}>
                        <Grid container spacing={3}>
                            {unitMembers.map(charaId => {
                                ...
                                console.log(member);
                                return (<Grid key={'chara' + charaId} xs={6} md={3} lg={3} item>
                                    <Link href={"/members/" + charaId}>
                                <Card  style={{ height: "100%"}} >
                                    <Grid container spacing={2}>
                                        {
                                            id !== "50" && <Grid item xs={12} style={{ backgroundImage: `url(${this.getIllustUrl(member)})`, height: '500px', backgroundPosition: "center top", backgroundSize: "cover", backgroundColor: member.ColorCode}}>
                                            </Grid>
                                        }
                                        <Grid item xs={12}>
                                            ...
                                            <Typography variant="caption" align="center" >
                                                {member.FullNameEnglish || member.FirstNameEnglish }
                                            </Typography>
                                        </Grid>
                                    </Grid>
                                </Card>
